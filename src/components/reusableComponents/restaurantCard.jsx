...
function RestaurantCard({ title, showDelivery, pages }) {
  ...
  const restaurantpics = useMemo(
    () =>
      shuffle(["bg-burger-bg", "bg-ramen-bg", "bg-pizza-bg", "bg-cuisine-bg"]),
    []
  );
        ...
        {pagesshuffled?.map((item, idx) => {
          return (
            <div className="flex flex-col w-full gap-2" key={idx}>
              ...
              >
                <div
                  className={`h-[160px] rounded-lg ${restaurantpics[idx]} bg-cover bg-no-repeat bg-center`}
                ></div>
              </Link>
